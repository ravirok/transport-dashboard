...
// ─── Code Inspector / ATC / DGP Routes ───────────────────────────────────────

// Helper: fetch all records from an entity and filter by TRANSPORT field
async function fetchQualityData(entity, trkorr) {
  const data = await fetchFromSAP(
    `/sap/opu/odata/sap/Z_TRANSPORTS_lOG_SRV_SRV/${entity}?$format=json`
  );
  const all = Array.isArray(data) ? data : [];
  // Filter by TRANSPORT or TRKORR field
  return all.filter(r => (r.TRANSPORT || r.TRKORR || '') === trkorr);
}

// 5. GET /api/transports/:trkorr/ci — Code Inspector findings
app.get("/api/transports/:trkorr/ci", async (req, res) => {
  const { trkorr } = req.params;
  try {
    // Try dedicated CI entity first; fall back gracefully if not in the service
    const results = await fetchQualityData("CIFindings", trkorr)
      .catch(() => fetchQualityData("CodeInspector", trkorr))
      .catch(() => []);

    console.log(`✅ CI findings for ${trkorr}: ${results.length}`);
    res.json({ d: { results } });
  } catch (err) {
    console.warn(`⚠️  CI endpoint not available for ${trkorr}:`, err.message);
    res.json({ d: { results: [] } }); // return empty — not a hard failure
  }
});

// 6. GET /api/transports/:trkorr/atc — ATC check results
app.get("/api/transports/:trkorr/atc", async (req, res) => {
  const { trkorr } = req.params;
  try {
    const results = await fetchQualityData("ATCFindings", trkorr)
      .catch(() => fetchQualityData("ATCResults", trkorr))
      .catch(() => []);

    console.log(`✅ ATC findings for ${trkorr}: ${results.length}`);
    res.json({ d: { results } });
  } catch (err) {
    console.warn(`⚠️  ATC endpoint not available for ${trkorr}:`, err.message);
    res.json({ d: { results: [] } });
  }
});

// 7. GET /api/transports/:trkorr/dgp — Downgrade Protection results
app.get("/api/transports/:trkorr/dgp", async (req, res) => {
  const { trkorr } = req.params;
  try {
    const results = await fetchQualityData("DGPResults", trkorr)
      .catch(() => fetchQualityData("DowngradeProtection", trkorr))
      .catch(() => []);

    console.log(`✅ DGP results for ${trkorr}: ${results.length}`);
    res.json({ d: { results } });
  } catch (err) {
    console.warn(`⚠️  DGP endpoint not available for ${trkorr}:`, err.message);
    res.json({ d: { results: [] } });
  }
});
...
Code Inspector (CI) results:
  Total findings : ${p.ci?.total    || 0}
  Errors         : ${p.ci?.errors   || 0}  ← error-level violations (highest severity)
  Warnings       : ${p.ci?.warnings || 0}
  Info           : ${p.ci?.infos    || 0}

ATC (ABAP Test Cockpit) results:
  Total findings : ${p.atc?.total  || 0}
  Priority 1     : ${p.atc?.prio1  || 0}  ← critical, must fix before release
  Priority 2     : ${p.atc?.prio2  || 0}  ← important warnings
  Exemptions     : ${p.atc?.exempt || 0}  ← findings with approved exemptions

Downgrade Protection (DGP) results:
  Objects checked  : ${p.dgp?.total      || 0}
  Violations       : ${p.dgp?.violations || 0}  ← objects that would OVERWRITE newer production versions
  Warnings         : ${p.dgp?.warnings   || 0}

Scoring rules:
  - Return an integer riskScore between 5 and 98.
  - "Failed" transport status is a strong risk signal (+35).
  ...
  - CI errors are blocking issues (+15 each, max +30). CI warnings add moderate risk (+5 each, max +15).
  - ATC priority-1 findings are critical blockers (+20 each, max +40). Priority-2 adds risk (+8 each, max +16).
  - DGP violations are extremely high risk — overwriting newer production code/data (+25 each, max +50).
  - reasoning must be ONE sentence, max 20 words, explaining the single most important risk driver across all checks.

...
async function resolveDeployment(baseUrl, token, resourceGroup) {
  ...
  // Pick first running deployment
  const dep        = deployments[0];
  const deploymentId = dep.id;
  // Model name lives in details.resources.backendDetails.modelName or similar paths
  const modelName  =
...
/**
 * Fetch the model name for a known deployment ID from the management API.
 * Only called when AICORE_DEPLOYMENT_ID is set but AICORE_MODEL_NAME is not.
 */

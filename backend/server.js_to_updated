...
// 2. GET /api/transports/:trkorr/objects — failed objects for a transport
app.get("/api/transports/:trkorr/objects", async (req, res) => {
  try {
    const { trkorr } = req.params;
    console.log(`🔄 Fetching objects for: ${trkorr}`);
    const data = await fetchFromSAP(
      `/sap/opu/odata/sap/Z_TRANSPORTS_lOG_SRV_SRV/Transports('${encodeURIComponent(trkorr)}')/Objects?$format=json`
    );
    const results = Array.isArray(data) ? data : [];
    console.log(`✅ Fetched ${results.length} objects for ${trkorr}`);
    res.json({ d: { results } });
  } catch (err) {
...
// 3. GET /api/transports/:trkorr/logs — logs for a transport
app.get("/api/transports/:trkorr/logs", async (req, res) => {
  try {
    const { trkorr } = req.params;
    console.log(`🔄 Fetching logs for: ${trkorr}`);
    const data = await fetchFromSAP(
      `/sap/opu/odata/sap/Z_TRANSPORTS_lOG_SRV_SRV/Transports('${encodeURIComponent(trkorr)}')/Logs?$format=json`
    );
    const results = Array.isArray(data) ? data : [];
    console.log(`✅ Fetched ${results.length} logs for ${trkorr}`);
    res.json({ d: { results } });
  } catch (err) {

...
fetch('/api/transports')
  ...
  .then(data => {
    ...
    // Chart: Transport Status
    const ctx = document.getElementById('statusChart').getContext('2d');
    const statusCounts = { Success: 0, Failed: 0 };
    transports.forEach(t => { statusCounts[t.Status]++; });
    new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['Success', 'Failed'],
        datasets: [{ label: 'Transports', data: [statusCounts.Success, statusCounts.Failed], backgroundColor: ['#27ae60','#e74c3c']]}
      },
